...
namespace FourFathers
{
	...
	public sealed class NetworkedInput : NetworkBehaviour, IBeforeUpdate
	{
		public static float LookSensitivity = 1;

		private NetworkedInputStruct _accumulatedInput;
		...
		void IBeforeUpdate.BeforeUpdate()
		{
			...
			if (mouse != null)
			{
				...
				//Debug.Log(string.Format("Look Rotation: {0} : {1}", lookRotationDelta , mouseDelta));
			...
			if (keyboard != null)
			{
				...
				_accumulatedInput.Buttons.Set(EInputButton.Spray, keyboard.tKey.isPressed);
				_accumulatedInput.Buttons.Set(EInputButton.Crouch, keyboard.ctrlKey.isPressed);
				_accumulatedInput.Buttons.Set(EInputButton.Suicide, keyboard.kKey.isPressed);
				_accumulatedInput.Buttons.Set(EInputButton.Interact, keyboard.eKey.isPressed);
			}
		}
